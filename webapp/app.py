...
async def run_simulation(payload: dict = Body(...)):
    ...
    try:
        ...
        use_fixed_cost_capacity = bool(payload.get("use_fixed_cost_capacity", False))
        use_layoff_limit = bool(payload.get("use_layoff_limit", False))
        use_max_hours = bool(payload.get("use_max_hours", False))
        use_subcontracting = bool(payload.get("use_subcontracting", False))
        use_safe_stock = bool(payload.get("use_safe_stock", False))
        use_smooth = bool(payload.get("use_smooth", False))
        max_hours = float(payload.get("max_hours", 4160))
        cap_max = float(payload.get("cap_max", 1000000))
        fixed_cost = float(payload.get("fixed_cost", 1))
        layoff_pct = float(payload.get("layoff_pct", 0.10))
        csc = float(payload.get("csc", 100))
